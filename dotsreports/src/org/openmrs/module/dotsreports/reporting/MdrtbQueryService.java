...
public class MdrtbQueryService {
    ...
    public static Cohort getPatientsDiedByTB(EvaluationContext context,Date startDate, Date endDate)
    {
    	//Get concept ids for Query:
    	Integer causeOfDeath 	= Context.getService(TbService.class).getConcept(TbConcepts.CAUSE_OF_DEATH).getConceptId();
    	Integer causeNonTb 		= Context.getService(TbService.class).getConcept(TbConcepts.DEATH_BY_OTHER_DISEASES).getConceptId();
    	Integer txOutcome		= Context.getService(TbService.class).getConcept(TbConcepts.TB_TX_OUTCOME).getConceptId();
    	...
    	query.append("		and o2.value_coded != " + causeNonTb + ";");	//291
    	
    	return executeQuery(query.toString(), context);
    }
    //Added by Omar
    /**
     * @return the Cohort of patients who have died due to TB
     */
    public static Cohort getPatientsByCauseOfDeath(EvaluationContext context, Date minResultDate, Date maxResultDate, Concept... causeType) {
    	
    	Integer causeOfDeath = Context.getService(TbService.class).getConcept(TbConcepts.CAUSE_OF_DEATH[0]).getConceptId();
    	...
    	q.append("where 	p.patient_id = o.person_id ");
    	q.append("and	 	p.voided = 0 and o.voided = 0 ");
    	q.append("and		o.concept_id = " + causeOfDeath + " ");
    	addOptionalDateClause(q, "and o.obs_datetime >= ", minResultDate);
    	addOptionalDateClause(q, "and o.obs_datetime <= ", maxResultDate);
    	q.append("and o.value_coded in (");
		for (int i=0; i<causeType.length; i++) {
			q.append(causeType[i].getConceptId());
			if ((i+1)<causeType.length) {
				q.append(",");
			}
		}	
    	q.append(") ");
     ...
     * @return the Cohort of patients with DST rest results
     */
    public static Cohort getPatientsWithTxOutcome(EvaluationContext context, Date minResultDate, Date maxResultDate, Concept outcomeType) {
    	...
    	if(outcomeType!=null) {
    		q.append("and o.value_coded = " + outcomeType.getConceptId());
    	}
